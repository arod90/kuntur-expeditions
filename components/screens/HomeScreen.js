...
function FourWorlds({ items, onNavigate }) {
  ...
  return (
    <div ref={ref} style={{ display: "flex", gap: "clamp(12px,1.4vw,22px)", alignItems: "flex-start", paddingTop: "18px" }}>
      {items.map((r, i) => {
        const restY = i * 30;
        const restRot = (i - 1.5) * 1.7;
        const isHover = hover === i;
        const ty = shown ? (isHover ? restY - 16 : restY) : restY - 84;
        ...
        return (
          <button key={r.title} type="button"
            ...
            style={{
              flex: 1, minWidth: 0, padding: 0, border: "none", background: "none", cursor: "pointer",
            ...
            <div style={{
              ...
              <Image src={r.image} alt={r.title} fill sizes="(max-width: 820px) 50vw, 22vw" placeholder="blur" style={{ objectFit: "cover", objectPosition: r.pos, transform: isHover ? "scale(1.05)" : "scale(1)", transition: "transform 900ms var(--ease-out)" }} />
    ...
    <div>
      <div style={{ position: "relative" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 2, pointerEvents: "none", color: "var(--paper-100)" }}>
          ...
          <h1 style={{
            position: "absolute", left: 0, right: 0, bottom: "53%", margin: 0, textAlign: "center",
      ...
      <Section tint="var(--ink-900)">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: "clamp(12px,1.4vw,22px)", height: "clamp(260px, 34vh, 420px)" }}>
        ...
        <div style={{ marginTop: "clamp(12px,1.4vw,22px)", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "clamp(12px,1.4vw,22px)", height: "clamp(180px, 24vh, 280px)" }}>
      ...
      {/* How it works */}
      <Section>
        <div style={{ display: "grid", gridTemplateColumns: "0.9fr 2.4fr", gap: "clamp(24px,3vw,64px)", alignItems: "stretch" }}>
          <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderTop: "1px solid var(--line-hair)" }}>
            {steps.map((s, idx) => (
              <div key={s.i} style={{ position: "relative", paddingTop: 28 }}>
                {idx > 0 ? <div style={{ position: "absolute", left: 0, top: 0, bottom: 0 }}><RevealLine orientation="vertical" delay={idx * 90} /></div> : null}
